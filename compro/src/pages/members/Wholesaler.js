...
const Wholesaler = () => {
    ...
    useEffect(() => {
        axios.get(`https://pacific-hamlet-79377.herokuapp.com/id/MembersWholesaler`)
        .then((res) => {
            ...
            if (dataRaw) {
                ...
                const hero = {
                    ...
                    footer: 'ARE YOU COMPANY LOOKING TO EXPAND SELLING CHANNEL AND NETWORK?'
                }
                setHeroContent(hero)
                ...
                if (dataRaw.WhatWeOfferTitle) {
                    ...
                    for(let key in dataRaw.WhatWeOfferDescription) {
                        whatWeOffer[index].description = dataRaw.WhatWeOfferDescription[key]
                        index++
                    }
                ...
                if (dataRaw.KeyFeaturesForWholesalerOnlineBillingDocument) {
                    ...
                    for (let key in dataRaw.KeyFeaturesForWholesalerOnlineBillingDocument) {
                        dataOnlineBilling.push(dataRaw.KeyFeaturesForWholesalerOnlineBillingDocument[key])
                    }
                    console.log(dataOnlineBilling)
                    const dataSupportingTools = []
                    for (let key in dataRaw.KeyFeaturesForWholesalerSupportingTools) {
                        dataSupportingTools.push(dataRaw.KeyFeaturesForWholesalerSupportingTools[key])
                    }

                    const webPortalManagement = []
                    for (let key in dataRaw.KeyFeaturesForWholesalerWebPortalManagement) {
                        webPortalManagement.push(dataRaw.KeyFeaturesForWholesalerWebPortalManagement[key])
                    }
    ...
    return (
        <div className="members-page">
            ...
            <BenefitMembers textHeader={'FOR WHOLESALER'} textSubHeader={'WHAT WE OFFER'} data={offerContent}/>
            <SupplierWork title={'HOW WHOLESALER WORKS'} content={contentWork} workBackground={workBackground}/>
            <ProcessMembers processImg={processImage}/>
            <ConditionMembers data={conditionContent}/>
            <KeyFeatureMembers content={contentFeature} title={'KEY FEATURES FOR WHOLESALER'} />
        </div>
    )
}
